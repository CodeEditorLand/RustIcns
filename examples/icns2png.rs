...
fn main() {
	...
	let icns_file = BufReader::new(
		File::open(icns_path).expect("failed to open ICNS file"),
	);
	...
	let (icon_type, png_path) = if num_args == 3 {
		...
		let icon_type =
			IconType::from_ostype(ostype).expect("unsupported ostype");
	...
	let png_file = BufWriter::new(
		File::create(png_path).expect("failed to create PNG file"),
	);
